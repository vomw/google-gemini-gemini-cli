...
describe('sandbox', () => {
  ...
  describe('start_sandbox', () => {
    ...
    it('should pass through the internal stdin prompt marker', async () => {
      const config: SandboxConfig = createMockSandboxConfig({
        command: 'docker',
        image: 'gemini-cli-sandbox',
      });
      process.env['GEMINI_CLI_INTERNAL_STDIN_PROMPT_INJECTED'] = '1';
      ...
      expect(spawn).toHaveBeenCalledWith(
        ...
        expect.arrayContaining([
          '--env',
          'GEMINI_CLI_INTERNAL_STDIN_PROMPT_INJECTED=1',
        ]),
        expect.any(Object),
      );
    });
