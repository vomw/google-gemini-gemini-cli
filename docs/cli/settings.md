...
| Tool Call Timeout                | `tools.callTimeout`                  | Maximum time in seconds that a single tool call may run before it is aborted. Set to 0 (the default) or a negative value to disable. Applies to all tool calls; for shell commands it works alongside tools.shell.inactivityTimeout. | `0`     |
